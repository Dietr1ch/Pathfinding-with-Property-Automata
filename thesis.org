...
# that possibly holds duplicates,
# but allows distinguishable elements.
#+LATEX: \end{definition}
...
When label sets $\Sigma$ or $\Tau$ are omitted, we assume that the corresponding
identity labeling is being used.
We call  $\Gr^\Tau$ an /edge-labeled Graph/, and
				 $\Gr_\Sigma$ a /vertex-labeled Graph/.
#+LATEX: \end{definition}

To quickly refer to labels in a graph, we use the /triple notation/.
We say that $(\sigma_u, \tau, \sigma_v) \in \LG$ if there is an edge $e=(u, v)$
in the graph such that $\sigma_u, \sigma_v \in \Sigma$ are the labels for $u$
and $v$ respectively.
...
#+begin_export latex
\begin{align*}
paths(\Gr) = \set{\pi \in \Seq{E} \mid \forall\ i\geq 0\; snd(\pi_i)=fst(\pi_{i+1})}
\end{align*}
#+end_export
#+LATEX: \end{definition}
