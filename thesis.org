...
#+end_export
** Classic Automata and our definition
The classic, deterministic automata definition is by a tuple $(S, s_0, \Sigma,
\delta, F)$ where $S$ is the set of /States/ and $\delta \colon S \times \Sigma
\to S$ is a function fixing the transitions of the machine.

Definitions only differ on $\delta$ and $(E, l)$ respectively, but it's easy to
see the bijection between both when the automata happens to be deterministic, as
otherwise $\delta$ cannot be a complete function.

Note that some automatons can be extended with a /sink/ state, to mention all
labels, and making $\delta$ complete.

*** Classic automata to ours
 $\delta$ induces the /edges/ and their /labels/. A sample mapping given by
 $\delta$ can be the following, $\delta(u, a) = v$. This requires that an edge
 $e\in E$ from $u$ to $v$ exist, and that it's assigned label is $a$, so $l(e)=a$.

*** Our automata to the classical automata
Given any edge $e=(u,v)$ on $E$, $\delta$ should map $(u, l(e))$ to $v$.
