...
/IRIs/ are special as they allow to retrieve information from the web by
/dereferencing/, which is the process of requesting documents on the web.

#+LATEX: \begin{definition}[RDF graph]
Using the /RDF sets/ a labeled graph is built. An /RDF graph/ is an
/edge-labeled graph/ over $V = \IRI \cup \Blank \cup \Lit$ with some
additional restrictions,

	- Edges cannot start on a literal.
	- Only /IRIs/ are used as labels, and their labeling is considered normal.
#+LATEX: \end{definition}

The use of a common set of /IRIs/ for vertices and labels is encouraged as it
allows joining information from multiple publishers, and also avoids duplication
and possibly inconsistent information on the graph.

On /RDF graphs/ tuple notation can be simplified as the graph has a normal labeling.
Simplified tuples are known as /RDF triples/, which are triples
 $(s, p, o)$ in $(V \setminus \Lit) \times \IRI \times V$.
RDF denotes members of those triples as /subjects/, /predicates/ and /objects/.

There is a trivial mapping between sets of /RDF triples/ and /RDF graphs/, this
allows servers to publish triples to create an universal, distributed graph made
of the union of all served triples.
This graph is known as the /Semantic Web Graph/ (SWG), and can be explored
through dereference. Every server will provide a document containing triples mentioning the
dereferenced IRI on the document as the /subject/ or the /object/.
